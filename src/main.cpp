...
void setup ()
{
  ...
  {
    mensaje_iniciar_sistema (lcd);
    delay (250); // Espera 100 milisegundos antes de volver a verificar
  }

  mensaje_incio (lcd);                                                // MOSTRAR MENSAJE DE INICIO
...
void comparacion_mov (int num_estado_mov, String fila_mov, int bandera_movil_aux)
{
  ...
  {
    for ( uint8_t i = 0; i < sizeof (movil_puntero) / sizeof (movil_puntero[0]); i++ )
    {
      if ( fila_mov.equals (movil_puntero[i]) )
      {
        bandera_movil = 1;
#ifdef DEBUG
...
void comparacion_ced (int num_estado_aux, String cedula_fila_aux, int bandera_cedula_aux)
{
  ...
  {
    for ( uint8_t i = 0; i < sizeof (comparar) / sizeof (comparar[0]); i++ )
    {
      ...
      if ( cedula_fila_aux.equals (myString) )
      {
        bandera_cedula = 1;
